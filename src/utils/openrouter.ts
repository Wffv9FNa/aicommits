...
import type { ExtendedModel } from './ai-provider.js';
import pkg from 'https-proxy-agent';
const { HttpsProxyAgent } = pkg as { HttpsProxyAgent: any };
		...
		const request = https.request(
			{
				...
				timeout,
				agent: proxy ? (new HttpsProxyAgent(proxy) as unknown as Agent) : undefined,
			},
			(response) => {
				const body: Buffer[] = [];
